...
FROM debian:13-slim
RUN apt update && apt install -y sudo curl xclip tmux adduser && adduser --uid 1000 siuyin && adduser siuyin sudo  && echo "siuyin ALL=(ALL:ALL) NOPASSWD: ALL" | sudo tee /etc/sudoers.d/siuyin
...
COPY --chown=siuyin:siuyin amux.conf profile_paths .
